...
- **Roadmap**:
  ...
  least-conn), static files, SSE, TCP + UDP raw proxy
  (`amalgame-net-stream` v0.2.0 — `TcpProxy` + `UdpProxy`, fixed
  upstream). **Partial:** stream multi-upstream LB still TODO, gRPC
  unary, SMTP *client*. **Not built yet:** SMTP *server* / IMAP /
